...
export const aggregateServers = functions.firestore
	.document("Streamers/{docId}/discord/data")
...
export const aggregateRankCards = functions.firestore
	.document("Leveling/{serverId}/users/{userId}")
	.onWrite(async (change, context) => {
		return true;
		const { serverId, userId } = context.params;
		Logger.log(context.params, change);
		Logger.log(serverId, userId);
...
export const aggregateDiscordId = functions.firestore
	.document("Streamers/{userId}/discord/data")
	.onWrite(async (change, context) => {
